...
if ($wp_query->max_num_pages <= 1) return; $paged=get_query_var('paged') ? absint(get_query_var('paged')) : 1; $max=intval($wp_query->max_num_pages);
	...
	if (($paged + 2) <= $max) { $links[]=$paged + 2; $links[]=$paged + 1; } echo '<div class="pagination"><ul class="d-flex list-unstyled mx-auto">' . "\n" ; /** Previous Post Link */ if (get_previous_posts_link()) printf('<li class="arrow">%s</li>' . "\n", get_previous_posts_link('<i class="icon-seta_anterior"></i>'));
